...
import { formatNativeUnits, formatUsdSecondary } from '@/lib/format';
...
export function AccountPanel({
    ...
    return (
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <button
          ...
          className="w-full rounded bg-btc py-2 text-sm font-medium text-black transition-colors hover:bg-btc/80 disabled:opacity-50"
        >
  ...
  const capitalUsd =
    btcUsd && marketConfig.symbol === 'BTC'
      ? (Number(account.capital) / 10 ** marketConfig.decimals) * btcUsd
      : null;
      ...
      <div className="mb-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded border border-border bg-background px-3 py-2 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-btc"
          />
          <button
